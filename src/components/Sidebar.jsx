...
export const Sidebar = ({ isMobileOpen, onMobileClose }) => {
  ...
  const mainNavItems = [
    ...
    { name: 'AI Workspace', path: '/workspace', icon: Sparkles, badge: 'New' },
  ];

  const aiToolItems = [
    { name: 'Flashcards', path: '/flashcards', icon: Brain },
    { name: 'Quiz Generator', path: '/quiz', icon: HelpCircle },
    { name: 'Revision Notes', path: '/revision-notes', icon: BookOpen },
    { name: 'AI Glossary', path: '/glossary', icon: BookMarked },
    { name: 'AI Insights', path: '/insights', icon: BarChart2 },
    { name: 'My Notes', path: '/notes', icon: StickyNote },
    { name: 'Bookmarks', path: '/bookmarks', icon: Bookmark },
    { name: 'Compare PDFs', path: '/compare', icon: GitCompare },
  ];
      ...
      <div className="p-3.5 rounded-2xl bg-slate-100/80 dark:bg-slate-800/50 border border-slate-200/70 dark:border-slate-700/50 flex items-center space-x-3">
        <div className="relative">
          {user?.avatar ? (
            ...
            <div
              className="w-10 h-10 rounded-xl ring-2 ring-brand-500/40 flex items-center justify-center font-bold text-sm text-white"
        ...
        {aiToolItems.map((item) => {
          ...
          return (
            <NavLink
              ...
              onClick={onMobileClose}
              className={({ isActive }) =>
                `flex items-center space-x-3 px-3.5 py-2 rounded-xl font-medium text-sm transition-all duration-200 group ${
                  isActive
                    ? 'bg-gradient-to-r from-brand-500 to-brand-600 text-white shadow-md shadow-brand-500/25'
                    : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/80'
                }`
